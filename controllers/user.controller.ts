...
export const registrationUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Retornar sucesso com dados do usuário (sem enviar a senha)
      // res.status(201).json({
      //   success: true,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (newUser.activationCode !== activation_code) {
        return next(new ErrorHandler('codigo invalido', 400))
      }
      ...
      if (existUser) {
        return next(new ErrorHandler('Email ja existe', 400))
      }
...
export const loginUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!user) {
        return next(new ErrorHandler('email ou senha invalidos', 400))
      }
...
export const updateAccessToken = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const message = 'Não foi possivel atualizar'
      ...
      if (!session) {
        return next(
          new ErrorHandler(
            'Por favor, faca login para acessar os recursos',
            400
          )
        )
      }
      ...
      await redis.set(user._id, JSON.stringify(user), 'EX', 5)
